...
from cloud_agent.StatusReporter import StatusReporter
...
class AutoRecoverySystem:
    ...
    def _handle_connection_error(self, count, time_since):
        ...
        if count <= 3:
            ...
            wait_time = min(count * 5, 15)  # 5, 10, 15 seconds
            action = f"retry after {wait_time}s"
        elif time_since < 300:  # 5 minutes frequent errors
            wait_time = min(30 * (count - 2), 300)  # max 5 minutes
...
class AgentCore:
    ...
    def _check_sudo_permissions(self):
        ...
        if os.geteuid() == 0:  # root user no need to check
            return
            ...
            for server_hostname in server_hostnames:
                ...
                try:
                    ...
                    while retries < max_retries and not self.stop_event.is_set():
                        ...
                        wait = min(2 ** retries, 30)  # 指数避退, 上限30                    
                        time.sleep(wait)
    ...
    def _start_core_threads(self):
        ...
        status_reporter = StatusReporter(self.initializer.config, self.stop_event)

        status_reporter.start()

        self.logger.info(f"Started thread: {status_reporter.name}")
