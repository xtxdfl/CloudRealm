...
class TestMonitorWebserverResource(TestCase):
  ...
  def test_setup_redhat(self, is_redhat_family, is_ubuntu_family, is_suse_family):
    ...
    expected_resources = (
      ...
      "/etc/httpd/conf/httpd.conf && cloud-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E sed -i "
      "'s/KeepAlive Off/KeepAlive On/' /etc/httpd/conf/httpd.conf || echo 'KeepAlive On' "
      "| cloud-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E tee --append /etc/httpd/conf/httpd.conf > /dev/null']"
  ...
  def test_setup_suse(self, is_redhat_family, is_ubuntu_family, is_suse_family):
    ...
    expected_resources = (
      "[MonitorWebserver['start'], Execute['grep -E 'KeepAlive (On|Off)' "
      "/etc/apache2/httpd.conf && cloud-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E sed -i "
      "'s/KeepAlive Off/KeepAlive On/' /etc/apache2/httpd.conf || echo 'KeepAlive On' "
      "| cloud-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E tee --append /etc/apache2/httpd.conf > /dev/null'],"
