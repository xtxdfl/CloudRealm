...
sys.path.append("/usr/local/cloud-agent/lib/")
...
from cloud_commons.os_family_impl import OsFamilyImpl, OsFamilyFuncImpl
...
RUN_HOST_CHECKS_CMD = "/usr/local/cloud-agent/cache/custom_actions/scripts/check_host.py ACTIONEXECUTE {0} /usr/local/cloud-agent/cache/custom_actions {1} INFO {2}"
...
REPOSITORY_BLACK_LIST = ["cloud.repo"]

PACKAGES_BLACK_LIST = ["cloud-server", "cloud-agent"]
...
class HostCleanup:
  ...
  def resolve_cloud_config(self):

    try:

      config = CloudConfig()

      if os.path.exists(CloudConfig.getConfigFile()):

        config.read(CloudConfig.getConfigFile())
  ...
  def run_os_command(self, cmd, runWithSudo=True):
    ...
    if runWithSudo:
      ...
      cmd = "/usr/local/cloud-agent/" + cloud_SUDO_BINARY + " " + cmd
