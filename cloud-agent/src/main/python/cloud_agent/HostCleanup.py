...
sys.path.append("/usr/lib/cloud-agent/lib/")
########################################################
...
RUN_HOST_CHECKS_CMD = "/var/lib/cloud-agent/cache/custom_actions/scripts/check_host.py ACTIONEXECUTE {0} /var/lib/cloud-agent/cache/custom_actions {1} INFO {2}"
...
class HostCleanup:
  ...
  def run_os_command(self, cmd, runWithSudo=True):
    if runWithSudo:
      cmd = "/var/lib/cloud-agent/" + cloud_SUDO_BINARY + " " + cmd
