...
class TestRepositoryResource(TestCase):
  ...
  def test_create_repo_redhat(
    ...
    with Environment("/") as env:
      with patch.object(
        repository, "__file__", new="/cloud/test/repo/dummy/path/file"
      ):
        Repository(
          "hadoop",
  ...
  @patch(
    "cloud_commons.os_utils.current_user", new=MagicMock(return_value="cloud-agent")
  )
  def test_create_repo_ubuntu_repo_exists(
    ...
    self.assertEqual(
      copy_item0,
      "call('/tmp/1.txt', content=StaticFile('/etc/apt/sources.list.d/HDP.list'), owner='cloud-agent')",
    )
    self.assertEqual(
      copy_item1,
