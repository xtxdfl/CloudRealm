...
from cloud_commons.cloud_service import CloudService, ENV_PYTHON_PATH
from cloud_commons.exceptions import *
from cloud_commons.logging_utils import *
from cloud_commons.os_windows import WinServiceController
from cloud_commons.os_utils import find_in_path
from cloud_agent.CloudConfig import CloudConfig, updateConfigServerHostname
from cloud_agent.HeartbeatHandlers import HeartbeatStopHandlers
...
def parse_options():
  # parse env cmd
  with open(os.path.join(os.getcwd(), "cloud-env.cmd"), "r") as env_cmd:
  ...
  if "cloud_AGENT_CONF_DIR" not in os.environ:
    os.environ["cloud_AGENT_CONF_DIR"] = os.getcwd()
  if "cloud_AGENT_LOG_DIR" not in os.environ:
    os.environ["cloud_AGENT_LOG_DIR"] = os.path.join(
      "\\", "var", "log", "cloud-agent"
    )
  if not os.path.exists(os.environ["cloud_AGENT_LOG_DIR"]):
    os.makedirs(os.environ["cloud_AGENT_LOG_DIR"])
  if "PYTHON_EXE" not in os.environ:
    os.environ["PYTHON_EXE"] = find_in_path("python.exe")


class CloudAgentService(CloudService):
  CloudService._svc_name_ = "Cloud Agent"
  CloudService._svc_display_name_ = "Cloud Agent"
  CloudService._svc_description_ = "Cloud Agent"
  ...
  def _adjustPythonPath(self, current_dir):
    ...
    agent_path = os.path.join(current_dir, "sbin", "cloud_agent")
    iPos += 1
  ...
  def SvcDoRun(self):
    ...
    self.heartbeat_stop_handler = HeartbeatStopHandlers(
      CloudAgentService._heventSvcStop
    )
  ...
  def redirect_output_streams(self):
    self._RedirectOutputStreamsToFile(CloudConfig.getOutFile())
    pass
...
def ctrlHandler(ctrlType):
  CloudAgentService.DefCtrlCHandler()
  return True


#
# Configures the Cloud Agent settings and registers the Windows service.
#
def setup(options):
  config = CloudConfig()
  # TODO cloud-18733, need to read home_dir to get correct config file.
  configFile = config.getConfigFile()

  updateConfigServerHostname(configFile, options.host_name)

  CloudAgentService.set_ctrl_c_handler(ctrlHandler)
  CloudAgentService.Install()


#
# Starts the Cloud Agent as a service.
# Start the Agent in normal mode, as a Windows service. If the Cloud Agent is
# not registered as a service, the function fails. By default, only one instance of the service can
#     possibly run.
#
def svcstart(options):
  (ret, msg) = CloudAgentService.Start(15)
  if 0 != ret:
...
#
# Stops the Cloud Agent.
#
def svcstop(options):
  (ret, msg) = CloudAgentService.Stop()
  if 0 != ret:
    options.exit_message = msg


#
# The Cloud Agent status.
#
def svcstatus(options):
  options.exit_message = None

  statusStr = CloudAgentService.QueryStatus()
  print("Cloud Agent is " + statusStr)
...
def svcdebug(options):
  ...
  CloudAgentService.set_ctrl_c_handler(ctrlHandler)
  win32serviceutil.HandleCommandLine(CloudAgentService, options)
...
def init_options_parser():
  ...
  parser.add_option(
    ...
    dest="host_name",
    default="localhost",
    help="Use specified Cloud server host for registration.",
  )
  parser.add_option(
    "-j",
    "--java-home",
...
def agent_main():
  ...
  options.exit_message = f"Cloud Agent '{action}' completed successfully."
  try:
    ...
    if options.warnings:
      for warning in options.warnings:
        ...
        pass
      options.exit_message = f"Cloud Agent '{action}' completed with warnings."
      pass
