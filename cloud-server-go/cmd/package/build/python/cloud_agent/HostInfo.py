...
from cloud_commons.firewall import Firewall
from cloud_commons.os_family_impl import OsFamilyImpl
from resource_management.core import shell

from cloud_agent.HostCheckReportFileHandler import HostCheckReportFileHandler
...
from cloud_agent import Hardware
...
class HostInfoLinux(HostInfo):
  ...
  def checkUnlimitedJce(self):
    if not self.config or not self.config.has_option(
      CloudConfig.cloud_PROPERTIES_CATEGORY, "java.home"
    ):
      return None
    try:
      jcePolicyInfo = JcePolicyInfo(
        self.config.get(CloudConfig.cloud_PROPERTIES_CATEGORY, "java.home")
      )
...
class HostInfoWindows(HostInfo):
  ...
  def createAlerts(self, alerts):
    # TODO cloud-7849 Implement createAlerts for Windows
    return alerts

  def javaProcs(self, list):
    try:
      from cloud_commons.os_windows import run_powershell_script
