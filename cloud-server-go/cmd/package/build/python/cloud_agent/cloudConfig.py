...
[agent]
prefix={ps}tmp{ps}cloud-agent
tmp_dir={ps}tmp{ps}cloud-agent{ps}tmp
...
[python]
custom_actions_dir = {ps}var{ps}lib{ps}cloud-agent{ps}resources{ps}custom_actions
...
[security]
keysdir={ps}tmp{ps}cloud-agent
server_crt=ca.crt
passphrase_env_var_name=cloud_PASSPHRASE
...
class CloudConfig:
  ...
  cloud_PROPERTIES_CATEGORY = "agentConfig"
  ...
  def getConfigFile(home_dir=""):
    ...
    """
    if "cloud_AGENT_CONF_DIR" in os.environ:
      return os.path.join(os.environ["cloud_AGENT_CONF_DIR"], "cloud-agent.ini")
    else:
      # home_dir may be an empty string
      return os.path.join(
        os.sep, home_dir, "etc", "cloud-agent", "conf", "cloud-agent.ini"
      )
  ...
  @property
  def cache_dir(self):
    return self.get("agent", "cache_dir", default="/var/lib/cloud-agent/cache")
  ...
  def command_file_retention_policy(self):
    ...
    * remove_on_success - Remove command-*.json files if the operation was successful
    ...
    The policy value is expected to be set in the Cloud agent's cloud-agent.ini file, under the
    [agent] section.
  ...
  # TODO cloud-18733, change usages of this function to provide the home_dir.
  @staticmethod
  def getLogFile(home_dir=""):
    ...
    """
    if "cloud_AGENT_LOG_DIR" in os.environ:
      return os.path.join(os.environ["cloud_AGENT_LOG_DIR"], "cloud-agent.log")
    else:
      return os.path.join(
        os.sep, home_dir, "var", "log", "cloud-agent", "cloud-agent.log"
      )
  ...
  def getAlertsLogFile(home_dir=""):
    ...
    """
    if "cloud_AGENT_LOG_DIR" in os.environ:
      return os.path.join(os.environ["cloud_AGENT_LOG_DIR"], "cloud-alerts.log")
    else:
      return os.path.join(
        os.sep, home_dir, "var", "log", "cloud-agent", "cloud-alerts.log"
      )
  ...
  def getOutFile(home_dir=""):
    ...
    """
    if "cloud_AGENT_LOG_DIR" in os.environ:
      return os.path.join(os.environ["cloud_AGENT_LOG_DIR"], "cloud-agent.out")
    else:
      return os.path.join(
        os.sep, home_dir, "var", "log", "cloud-agent", "cloud-agent.out"
      )
  ...
  def update_configuration_from_metadata(self, reg_resp):
    if reg_resp and CloudConfig.cloud_PROPERTIES_CATEGORY in reg_resp:
      if not self.has_section(CloudConfig.cloud_PROPERTIES_CATEGORY):
        self.add_section(CloudConfig.cloud_PROPERTIES_CATEGORY)
      for k, v in reg_resp[CloudConfig.cloud_PROPERTIES_CATEGORY].items():
        self.set(CloudConfig.cloud_PROPERTIES_CATEGORY, k, v)
